# partition_utils.py — Version 2.3
# Traitement partitions musicales avec boutons YouTube
"""
v2.3 :
  - ordre_boutons (0=YouTube|Jouer ici, 1=Jouer ici|YouTube)
  - x negatif = depuis bord droit : bx = W - BW - abs(x%)/100*W
  - fond blanc supprime (toujours transparent)
  - transparence agit sur opacite des boutons (pas du fond)

v2.2 :
  - Correction transparence fond : Color(alpha=x) ignoré par certains lecteurs PDF
    -> remplacé par setFillAlpha()/setStrokeAlpha() (standard PDF, fiable)
    -> stroke=0 et setStrokeAlpha(0) pour supprimer toute bordure
...
version = ("partition_utils.py", "2.3")
...
def draw_button(c, x, y, width, height, color, text, sens_texte=0, alpha=1.0):
    ...
    c.roundRect(x_draw, y_draw, width, height, radius / 2, fill=1, stroke=0)

    circle_x = x_draw + radius + 5
    circle_y = y_draw + height / 2
    c.setFillColor(white)
    c.setFillAlpha(alpha)
    c.circle(circle_x, circle_y, radius, fill=1, stroke=0)
    c.setFillColor(color)
    c.setFillAlpha(alpha)
    tri_half = radius * 0.6
    path = c.beginPath()
...
def create_overlay(page_width, page_height, player_url, video_id,
                   ...
                   rotation=0, bord=0, sens_texte=0, ordre_boutons=0):
    ...
    Args:
        ...
        sens_texte    : 0=boutons a l endroit, 1=retourne 180 degres
        ordre_boutons : 0=YouTube|Jouer ici, 1=Jouer ici|YouTube
    ...
    # x : None=centre, positif=depuis gauche, negatif=depuis droite
    # ex: x=-2  -> boutons finissent a 2% du bord droit
    if x_pct is None:
        eff_x = (eff_w - total_width) / 2
    elif x_pct < 0:
        eff_x = eff_w - total_width - (abs(x_pct) / 100.0) * eff_w
    else:
        eff_x = (x_pct / 100.0) * eff_w
    ...
    # Pas de fond : les boutons flottent directement sur la page
    # La transparence agit sur l'opacite des boutons eux-memes (voir draw_button)

    # Opacite des boutons (transparence agit ici, pas sur un fond)
    btn_alpha = max(0.0, min(1.0, fond_opacite_pct / 100.0))

    # Ordre des boutons
    if ordre_boutons == 1:
        # Jouer ici | YouTube
        btn1_color, btn1_label, btn1_url = "#28a745", "Jouer ici", f"{player_url}?v={quote(video_id)}"
        btn2_color, btn2_label, btn2_url = "#FF0000", "YouTube",   f"https://youtube.com/watch?v={quote(video_id)}"
    else:
        # YouTube | Jouer ici
        btn1_color, btn1_label, btn1_url = "#FF0000", "YouTube",   f"https://youtube.com/watch?v={quote(video_id)}"
        btn2_color, btn2_label, btn2_url = "#28a745", "Jouer ici", f"{player_url}?v={quote(video_id)}"

    x2 = start_x + BUTTON_WIDTH + BUTTON_SPACING
    draw_button(c, start_x, start_y, BUTTON_WIDTH, BUTTON_HEIGHT,
                HexColor(btn1_color), btn1_label, sens_texte, btn_alpha)
    draw_button(c, x2, start_y, BUTTON_WIDTH, BUTTON_HEIGHT,
                HexColor(btn2_color), btn2_label, sens_texte, btn_alpha)
    ...
    rect2 = _link_rect(x2, start_y, BUTTON_WIDTH, BUTTON_HEIGHT,
                       rotation, page_width, page_height)
    c.linkURL(btn1_url, rect1, relative=0)
    c.linkURL(btn2_url, rect2, relative=0)
...
def ajouter_boutons_partition(source_pdf: Path, target_pdf: Path,
                               ...
                               orientation: str = "V",
                               fond_opacite_pct: float = 100.0,
                               pos_x: float = None,
                               pos_y: float = None,
                               bord: int = 0,
                               sens_texte: int = 0,
                               ordre_boutons: int = 0) -> bool:
        ...
        with open(csv_path, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                sens_texte    = _parse_int(_get_col(row,
                    "sens_texte", "sens"), 0)
                ordre_boutons = _parse_int(_get_col(row,
                    "ordre_boutons", "ordre"), 0)
                ...
                ordre_str = "Jouer ici|YouTube" if ordre_boutons == 1 else "YouTube|Jouer ici"
...
def params_signature(video_id: str, orientation: str, fond_opacite_pct: float,
                     pos_x, pos_y, bord: int = 0, sens_texte: int = 0,
                     ordre_boutons: int = 0) -> str:
    ...
    """
    return f"{video_id}|{orientation}|{fond_opacite_pct}|{pos_x}|{pos_y}|{bord}|{sens_texte}|{ordre_boutons}"
