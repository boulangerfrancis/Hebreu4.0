# documents.py — Version 1.7
# v1.7 : fond_opacite_pct remplace fond_transparent (passage à partition_utils v1.6)
...
def traiter_partitions_du_dossier(dossier: Path, log_func) -> int:
    ...
    for nom_pdf_final, params in correspondances.items():
        # Extraire paramètres (v1.4 : dict avec video_id + options)
        if isinstance(params, dict):
            ...
            # Compatibilité ancienne version (chaîne simple)
            video_id         = params
        ...
        # Lister tous les __partition_* PDF du dossier pour debug
        candidats = [f for f in dossier.iterdir()
                     if f.is_file() and f.suffix.lower() == ".pdf"
                     and (f.name.lower().startswith("__partition_") or
                          f.name.lower().startswith("__partition _"))]
