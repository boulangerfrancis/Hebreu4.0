# partition_utils.py — Version 1.2
...
def ajouter_boutons_partition(source_pdf: Path, target_pdf: Path, 
    ...
    print(f"  [DBG]   player_url : {player_url}", flush=True)
    print(f"  [DBG]   video_id   : {video_id}", flush=True)
    ...
    try:
        with open(csv_path, newline="", encoding="utf-8") as f:
            ...
            for i, row in enumerate(reader):
                ...
                print(f"  [DBG] Ligne {i+1} brute : pdf_name={pdf_name_brut!r}  url={youtube_url!r}", flush=True)
                ...
                print(f"  [DBG] Ligne {i+1} normalisée : {pdf_name_brut!r} → {pdf_name!r}", flush=True)
                ...
                print(f"  [DBG] Ligne {i+1} video_id : {video_id!r}", flush=True)
                correspondances[pdf_name] = video_id

        print(f"  [DBG] Correspondances chargées : {correspondances}", flush=True)
