...
def nom_partition_depuis_docx(nom_docx: str) -> Tuple[str, str]:
    """Calcule les noms dérivés d'un fichier __partition_*.docx.

    Exemple :
        entrée  : "__partition_blabla et blabla.docx"
        retours : ("blabla et blabla",          ← nom lisible (affiché)
                   "blabla_et_blabla.pdf")       ← nom normalisé (html/copie)

    Le préfixe "__partition_" est retiré. Les espaces sont conservés
    dans le nom lisible, normalisés en underscore dans le nom html.

    Args:
        nom_docx: Nom complet du fichier docx (avec préfixe __partition_)

    Returns:
        (nom_lisible, nom_html_normalise)
    """
    # Retirer le préfixe "__partition_" (avec underscore collé ou avec espace)
    ...
    stem = Path(sans_prefix).stem            # "blabla et blabla"
    nom_lisible = stem                        # conserve les espaces
    nom_html = normaliser_nom(stem) + ".pdf" # "blabla_et_blabla.pdf"
...
def traiter_partitions_du_dossier(dossier: Path, log_func) -> int:
    ...
    Chaîne complète pour "__partition_blabla et blabla.docx" :
      1. DOCX → "__partition_blabla et blabla.pdf"  (via traiter_docx_du_dossier)
      2. PDF source "__partition_blabla et blabla.pdf"
         → PDF final   "blabla_et_blabla.pdf"        (avec boutons YouTube)
      3. Seul "blabla_et_blabla.pdf" sera copié vers html/ (pas le __partition_*)

    Le fichier __correspondance.csv doit lister le nom html normalisé :
        pdf_name,youtube_url
        blabla_et_blabla.pdf,https://youtube.com/watch?v=XXXX
    ...
    for nom_pdf_final, video_id in correspondances.items():
        # Chercher le PDF source : __partition_*.pdf dont le nom normalisé correspond
        pdf_source = None
        for f in dossier.iterdir():
            ...
            if f.suffix.lower() != ".pdf":
                continue
            # Accepter __partition_* avec ou sans espace après le préfixe
            nom_lower = f.name.lower()
            if not (nom_lower.startswith("__partition_") or
                    nom_lower.startswith("__partition _")):
                continue

            # Calculer le nom html que produirait ce fichier source
            _, nom_html_attendu = nom_partition_depuis_docx(
                f.stem + ".docx"  # reconstituer le nom docx pour réutiliser la fonction
            )
        ...
        # PDF final sans préfixe, dans le même dossier documents/
        ...
        if not partitions.doit_regenerer_partition(pdf_source, pdf_final):
            log_func(f"  ✓ Partition à jour : {nom_pdf_final}")
            continue

        log_func(f"  🎵 Partition : {pdf_source.name} → {nom_pdf_final} (YouTube: {video_id})")
...
def fichier_docx_existe_pour(fichier_pdf: Path, dossier: Path) -> bool:
    ...
    Sert à éviter d'inscrire dans STRUCTURE.py un PDF dont le DOCX
    source est déjà enregistré.
    Les __partition_*.docx sont exclus : leur PDF surchargé (sans préfixe)
    est géré séparément via l'entrée __partition_ dans STRUCTURE.py.
    ...
    for f in dossier.iterdir():
        if f.suffix.lower() not in ('.doc', '.docx'):
            continue
        # Ignorer les __partition_* (ils ont leur propre logique)
        nom_lower = f.name.lower()
...
def mettre_a_jour_structure(dossier: Path, log_func) -> Dict[str, Any]:
    ...
    for entry in entries:
        ...
        # PDF "surchargé" (ex: blabla_et_blabla.pdf) dont un __partition_*.docx
        # est la source → ignoré aussi (déjà représenté par l'entrée __partition_)
        if ext == "pdf":
