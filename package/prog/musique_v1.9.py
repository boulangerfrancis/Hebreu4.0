# musique.py — Version 1.9
# v1.9 : dossier_html normalise (suppression accents/espaces dans le chemin cible)
#         bouton place en haut a gauche par defaut (DEF_POS_X=0, DEF_POS_Y=page_height-hauteur)
# v1.8 : refactorisation complete
#   - partition_utils supprime, remplace par place_bouton_v01
...
# Import place_bouton_v01 depuis le meme dossier que musique.py
import importlib.util
_pbp_path = Path(__file__).parent / "place_bouton_v01.py"
if not _pbp_path.exists():
    raise FileNotFoundError(
        f"place_bouton_v01.py introuvable dans {Path(__file__).parent}\n"
        f"Copier place_bouton_v01.py dans le meme dossier que musique.py"
    )
_spec = importlib.util.spec_from_file_location("place_bouton_v01", _pbp_path)
_pbp  = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_pbp)
print(f"  [musique] place_bouton_v01.py charge depuis {_pbp_path}")
...
DEF_POS_X          = 0.0
DEF_POS_Y_SENTINEL = -1.0   # valeur speciale : calculer depuis haut de page
DEF_ROTATION       = "E"
DEF_ORIENT         = "2"
DEF_LARGEUR        = 160
DEF_HAUTEUR        = 35
            ...
            for i, row in enumerate(reader):
                ...
                params = {
                    ...
                    "pos_y"           : _float(_get(row, "position_verticale",   "pos_y",
                                                    "position_v", "y"),                     DEF_POS_Y_SENTINEL),
...
def traiter_partitions_du_dossier(dossier: Path, log_func) -> int:
    ...
    # Dossier HTML cible : recalcule depuis DOSSIER_HTML en normalisant
    # chaque segment du chemin relatif entre DOSSIER_DOCUMENTS et dossier
    # Ex: documents\Musique hébraïque -> html\musique_hebraique
    try:
        chemin_relatif = dossier.relative_to(Path(DOSSIER_DOCUMENTS))
        segments_normalises = [_normaliser(s) for s in chemin_relatif.parts]
        dossier_html = Path(DOSSIER_HTML).joinpath(*segments_normalises)
    except ValueError:
        # dossier n'est pas sous DOSSIER_DOCUMENTS : fallback str.replace
    ...
    for p in lignes:
        ...
        log_func(f"        source : {pdf_source}")
        log_func(f"        cible  : {pdf_cible}")

        try:
            angle_rot      = _pbp.parse_rotation(p["rotation"])
            vertical, _    = _pbp.parse_orient(p["orient_texte"])

            # Position par defaut : haut-gauche de la page
            # pos_x=0 -> bord gauche
            # pos_y=None -> calculer depuis la hauteur reelle de la page
            pos_x = p["pos_x"]
            pos_y = p["pos_y"]
            if pos_y is None or pos_y == DEF_POS_Y_SENTINEL:
                # Lire la hauteur de la page pour placer en haut
                from pypdf import PdfReader as _PR
                _ph = float(_PR(str(pdf_source)).pages[0].mediabox.height)
                pos_y = _ph - p["hauteur"]
                log_func(f"        pos_y auto (haut) : page_h={_ph:.0f} - hauteur={p['hauteur']} = {pos_y:.0f}")

            # Log equivalent ligne de commande pour test manuel :
            log_func(f"        => equivalent commande :")
            log_func(f"           python place_bouton_v01.py"
                     f" \"{pdf_source.parent}\" \"{pdf_source.name}\""
                     f" \"{pdf_cible.parent}\" \"{pdf_cible.name}\""
                     f" \"{p['youtube_url']}\""
                     f" {p['transparence']}"
                     f" {pos_x} {pos_y}"
