# partition_utils.py — Version 2.2
...
version = ("partition_utils.py", "2.2")
...
def create_overlay(page_width, page_height, player_url, video_id,
    ...
    # Fond blanc avec transparence reelle (setFillAlpha = standard PDF)
    # Color(alpha=x) est ignore par certains lecteurs -> setFillAlpha() est fiable
    alpha = max(0.0, min(1.0, 1.0 - fond_opacite_pct / 100.0))
    if alpha > 0.005:
        pad = FOND_PADDING
        c.setFillColorRGB(1, 1, 1)   # blanc
        c.setFillAlpha(alpha)          # opacite : 1.0=opaque, 0.0=invisible
        c.setStrokeAlpha(0.0)          # pas de bordure
        c.rect(start_x - pad, start_y - pad,
               total_width + 2 * pad, BUTTON_HEIGHT + 2 * pad,
               fill=1, stroke=0)
        c.setFillAlpha(1.0)            # restaurer opacite pleine pour les boutons
